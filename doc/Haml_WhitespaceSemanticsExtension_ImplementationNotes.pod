...
=head2 RSpec and Test::More

A suite of specifications and tests accompany this text.

The file B<00ImplementationNotes> contains the snippets found in this
text, pretty much as shown ... well, that was the plan.

The files numbered from B<01> ro B<14> are by topic, contain specifications
...
By use of indentation to demarcate the extent of model content.
...
An extension of C<option:preserve>, with Mixed Content, without newline 
transforms. Includes a partner C<filter:preformatted>. In WSE Haml, 
by default, C<%pre> and C<%textarea> shift from C<option:preserve> 
to C<option:preformatted>. Authors will add to C<option:preformatted> any
tags for which they assign CSS C<white-space:pre>-like mechanics.
...
processing be represented (here, encoded) in such a way as to I<escape> 
such syntax processing. Thus, B<in Html>, considering two separate 
encodings of the ampersand, I<&> is a symbol susceptible to Html syntax 
...
(C<~ expr> and C<& expr>) and helper functions such as 
C<helper:capture_html>, operating individually or in composition.
...
immediate ContentBlock>. Under I<OIR:strict>, the following is allowed. 
...
including a difference under C<option:preserve> or C<option:preformatted>.
...
I<Nested Text Content> (which may enforce a single BlockLeftMargin, or
...
Head is listed in C<option:preserve>, the standard preserve transformations 
...
C<option:preformatted>, within filter:preserve, and in HereDoc; 
...
=item hamltag:%ptag, where C<ptag> is listed in option:preserve
...
I<reencode_interior_newlines_occurring_between_listed_html_elements>),
and the B<General Newline Transform facility> for Inline or Nested Content.
...
=item * How are newlines transformed? A.k.a: How is I<preserve> accomplished?

=item * How is whitespace transformed in from HamlSource to HtmlOutput?
...
(i.e., C<%atag> is replayed as the string I<%atag>).
...
extension in WSE Haml is to replay the author's input, I<preserving> the
Initial Whitespace:
...
Preformatted mechanics will run the element's content I<nested>.
...
=item BlockLeftMargin (BLM)

The offset indentation that serves as the point of reference for
I<Offside>: the minimum Indentation at which a Textline is considered
...
The plan is that the first, I<OOImplementationNotes>, contain all the code,
...
I<00ImplementationNotes>, and with each other on common topics. These
...
in this document, and are B<not> included in the I<00> file.

=over

=item - 00ImplementationNotes_spec.rb

=item - 01helpers_spec.rb

=item - 02initialwspc_spec.rb

=item - 03nesting_spec.rb

=item - 04preserve_spec.rb

=item - 05preformatted_spec.rb
