...
C<perl t/wse/00ImplNotes_Code09_05-10.t>.

These may be run in suites through use of the provided Makefile, using
the form C<make code_9_5>, for example. You may run the entire
00ImplNotes_Code suite with C<make implnotes>. Read t/wse/Makefile.

A Ruby RSpec collection is also provided. For example,
C<spec --color spec/00ImplNotes_Code09_5-10_spec.rb -f s>.
A Rakefile is also provided, to run spec suites. For example,
C<rake spec:suite:code_9_5>. You may run the entire RSpec 00ImplNotes_code
suite using C<rake spec:implnotes>. See the Rakefile.
...
the form C<rake spec:suite:code_9_5>, for example.
